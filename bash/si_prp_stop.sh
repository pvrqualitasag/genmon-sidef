#!/bin/bash
#' ---
#' title: Stop Singularity Container Instance For PopRep
#' date:  2020-06-18 15:32:43
#' author: Peter von Rohr
#' ---
#' ## Purpose
#' Make sure to stop the singularity instance together with the database.
#'
#' ## Description
#' Stopping a singularity container image that runs a postgresql-db for poprep.
#'
#' ## Details
#' To get a clean stop of the instance of the singularity container, we first stop the pg-db-server which increases the chance to leave the db in a consistent state.
#'
#' ## Example
#' ./si_prp_stop.sh -i siprp 
#'
#' ## Set Directives
...
singularity exec instance://$SINGULAIRTYINSTANCENAME $INSTALLDIR/pg_stop.sh
sleep 2
