...
PopRep is used as a command-line tool as user 'zws' on the server 'fagr.genmon.ch'. The complete PopRep system is installed in a singularity container image. Under user 'zws' an instance called 'siprp' of the singularity container is running. PopRep can only be run using the instance 'siprp' This can be done either via 
...
If the instance 'siprp' is not running, it can be started with the script '/home/quagadmin/simg/genmon-sidef/bash/si_prp_start.sh'. This script starts an instance of the singularity image and automatically starts the postgresql database which is required for running PopRep. A running singularity instance can be stopped with the script '/home/quagadmin/simg/genmon-sidef/bash/si_prp_stop.sh'.  
