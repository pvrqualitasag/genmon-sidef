...
From the image file that was built, the container is started with the bash script `start_si_gnm.sh`.

```
cd 
./simg/genmon-sidef/bash/start_si_gnm.sh -n /home/gnmzws/simg/img/genmon/gnm.sif
```


## Post-Building Steps
The Post-Building steps are done using the script `post_install_gnm.sh`. 
