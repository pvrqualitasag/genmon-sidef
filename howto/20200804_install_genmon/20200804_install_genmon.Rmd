...
The option '--writable' is no longer available for the 'singularity build' command. The way to create a changeable image is to create a sandbox directory with the '--sandbox' option. Once all the changes are done in that directory it can be converted to a '.sif' image file using the following command
